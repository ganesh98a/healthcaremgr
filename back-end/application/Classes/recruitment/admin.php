...
class AdminRecruit extends adminRoles\Roles {
    ...
    public function insertPhone() {
        ...
        if (count($this->secondary_phones) > 0) {
            foreach ($this->secondary_phones as $key => $val) {
                if ($key == 0) {
                    $addional_phone_number[] = array('phone' => $val->name, 'memberId' => $this->adminid, 'primary_phone' => 1);
                } else {
                    $addional_phone_number[] = array('phone' => $val->name, 'memberId' => $this->adminid, 'primary_phone' => 2);
                }
            }
    ...
    public function updateRoleToAdmin() {
        ...
        if (!empty($this->getRoles())) {
            ...
            $check_pin = $CI->basic_model->get_row('admin', array('pin'), $where = array('id' => $this->adminid));
    ...
    public function updatePinAdmin() {
        ...
        $CI->basic_model->update_records('admin', array('pin' => $encrypted_pin), $where = array('id' => $this->adminid));
    }
    ...
    function UpdatePrimaryEmail() {
        ...
        $where = array('adminId' => $this->getAdminid(), 'primary_email' => 1);

        $CI->basic_model->update_records('admin_email', array('email' => $this->primary_email), $where);
        
        $CI->basic_model->update_records('admin', array('token' => ''), array('id' => $this->getAdminid()));
    }

    function insertStaffDetail() {
        $CI = & get_instance();
        $staff_detail = array('adminId' => $this->adminid, /*'departmentId' => $this->getDepartmentid()*/);
        $CI->basic_model->insert_records('recruitment_staff', $staff_detail, $multiple = false);
    }
