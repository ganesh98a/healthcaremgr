...
//        $CI->load->model('Admin_model');
        ...
        $response = $this->CI->Admin_model->check_admin_permission($adminId, $pemission_key);
    ...
    function get_all_permission($adminId) {
        $CI = & get_instance();
        $CI->load->model('admin_model');

        $permissions = array();
        $response = $CI->admin_model->get_admin_permission($adminId);

        if (!empty($response)) {
            foreach ($response as $val) {
                //$check_has_permission = $this->check_permission($adminId,$val->permission);
//              if(!empty($check_has_permission)){
