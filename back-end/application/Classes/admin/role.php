...
class Roles extends classPermission\Permission
{
    ...
    {
        $CI = &get_instance();
        $response = $CI->basic_model->get_record_where_orderby('role', $column = array('name', 'role_key', 'id', '1 as access'), $where = array('status' => 1, 'archive' => 0), 'weight', 'asc');
    ...
    public function getUserBasedRoles($all = false)
    {
		...
		$super_amdin = get_super_admins();
        if (in_array($this->getAdminId(), $super_amdin)) {
    ...
    function getAdminPreviousRoles()
    {
        $CI = &get_instance();
        return $CI->admin_model->get_alloted_to_admin_roles($this->getAdminId());
    }

    function check_valid_approval_pending_staff()
    {
        $CI = &get_instance();
        $CI->load->model('recruitment/Recruitment_user_management');
        return $CI->Recruitment_user_management->check_valid_approval_pending_staff($this->getAdminId());
    }    
}
