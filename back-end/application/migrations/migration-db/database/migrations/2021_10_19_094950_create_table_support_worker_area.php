...
class CreateTableSupportWorkerArea extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        if (!Schema::hasTable('tbl_finance_support_worker_area')) {
            Schema::create('tbl_finance_support_worker_area', function (Blueprint $table) {
                $table->increments('id');
                $table->string('title');
    ...
    {
        if (Schema::hasTable('tbl_finance_support_worker_area')) {
            Schema::dropIfExists('tbl_finance_support_worker_area');
        }
    }
}
