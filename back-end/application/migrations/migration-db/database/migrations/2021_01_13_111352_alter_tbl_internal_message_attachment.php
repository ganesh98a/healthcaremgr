...
class AlterTblInternalMessageAttachment extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::table('tbl_internal_message_attachment', function (Blueprint $table) {
            if (!Schema::hasColumn('tbl_internal_message_attachment', 'file_path')) {

                $table->text('file_path')->nullable()->after('filename');

            }
            if (!Schema::hasColumn('tbl_internal_message_attachment', 'file_type')) {

                $table->text('file_type')->nullable()->after('file_path');

            }
            if (!Schema::hasColumn('tbl_internal_message_attachment', 'file_size')) {

                $table->text('file_size')->nullable()->after('file_type');

            }
            if (!Schema::hasColumn('tbl_internal_message_attachment', 'file_ext')) {

                $table->text('file_ext')->nullable()->after('file_size');

            }
            if (!Schema::hasColumn('tbl_internal_message_attachment', 'aws_object_uri')) {

                $table->text('aws_object_uri')->nullable()->after('file_ext');

            }
            if (!Schema::hasColumn('tbl_internal_message_attachment', 'aws_response')) {

                $table->text('aws_response')->nullable()->after('aws_object_uri');

            }
            if (!Schema::hasColumn('tbl_internal_message_attachment', 'aws_uploaded_flag')) {

                $table->unsignedInteger('aws_uploaded_flag')->default(0)->nullable()->after('aws_response')->comment('1 - Yes / 0 - No');

            }
            if (!Schema::hasColumn('tbl_internal_message_attachment', 'aws_file_version_id')) {
            ...
            if (!Schema::hasColumn('tbl_internal_message_attachment', 'updated_at')) {
