...
class Finance_import_meta_data extends CI_Model {
    ...
    function read_data_from_data_sheet($tmp_f_name, $overwrite_item, $adminId) {
        ...
        if ($dataValidation && isset($dataValidation['status'])) {
            # Insert or update
            $data = $dataValidation['data'] ?? [];
            
            $result = $this->data_import($data, $adminId);

            if (isset($dataValidation['status']) && $dataValidation['status'] == true) {
                if ($result == true) {                
    ...
    function update_bulk_import_line_item($data,$adminId) {
        ...
        if ($line_item_id > 0) {
            ...
            unset($list_item['line_item_id']);
            $list_item['updated_by'] = $adminId;
            $list_item['updated_at'] = DATE_TIME;
            
            return $this->basic_model->update_records('finance_line_item', $list_item, ['id' => $line_item_id]);
        }        
    }
    ...
    function validate_data($rows, $overwrite_item, $adminId) {
        ...
        foreach ($rows as $row) {
            ...
            if (!empty($priceListLI)) {
                ...
                $header_name = $this->get_raw_header_err_index($in_su_item_number, $header, $raw_header);
                $line_item_invalid[$header_name]['header'] = $header_name;
                if (isset($line_item_invalid[$header_name]['rows']) && count($line_item_invalid[$header_name]['rows'])) {
                    $count_row = count($line_item_invalid[$header_name]['rows']);
                    $line_item_invalid[$header_name]['count'] = $count_row;
                    $line_item_invalid[$header_name]['header'] = $header_name." ({$count_row} records)";
                } else {
                    $line_item_invalid[$header_name]['header'] = $header_name." (1 records)";
                }
                $line_item_invalid[$header_name]['rows'][] = $ct_in;
                
                $line_item_invalid[$header_name]['options'] = [];
                $line_item_invalid[$header_name]['error'] = [ $header_name.' not in the system' ];
            }            
        }
