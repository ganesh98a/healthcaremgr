...
if (!function_exists('generate_token')) {
    function generate_token($token_value) {
        $Obj_JWT = new \JWT();
        $JWT_Token = $Obj_JWT->encode($token_value, JWT_SECRET_KEY);
        return $JWT_Token;
    }
}
if (!function_exists('auth_login_status')) {
    ...
    function auth_login_status($login_detail) {
        ...
        $CI = & get_instance();
        $CI->load->model('Recruitment_device_model');
        try {
            $response = array();
            if (empty($login_detail)) {
                ...
                if (empty($device_token) || empty($applicant_id)) {
                    $response = array('status' => false, 'error' => system_msgs('empty_token_applicant'));
                    echo json_encode($response);
                    exit;
                } else {                    
                    $response=$CI->Recruitment_device_model->verify_ipad_token((array)$login_detail);
                    if (!$response) {                        
...
if(!function_exists('auth_login_status_file_type')){
    ...
    function auth_login_status_file_type(){
        ...
        if (empty($token) || empty($applicant_id)){
            ...
            $response=$CI->Recruitment_device_model->verify_ipad_token($login_detail);            
            if (!$response) {                        
                $response = array('status' => false, 'error' => 'Token expired or Mismatch');
                echo json_encode($response);
                exit;
